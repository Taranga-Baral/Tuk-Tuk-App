...
class _DriverAcceptedPageState extends State<DriverAcceptedPage> {
  ...
  void initState() {
    super.initState();
    _loadButtonState();  // Load button state from Firebase when the app is opened
  }

  Future<void> _loadButtonState() async {
    final buttonStates = await _fetchButtonStates();  // Fetch from Firebase
    setState(() {
      isSendButtonPressed = buttonStates['isSendButtonPressed'] ?? false;
      isDoneButtonPressed = buttonStates['isDoneButtonPressed'] ?? false;
    });
  }

  Future<Map<String, bool>> _fetchButtonStates() async {
    final tripDoc = await FirebaseFirestore.instance.collection('buttonStates').doc(widget.driverId).get();
    return tripDoc.exists
        ? {
            'isSendButtonPressed': tripDoc['isSendButtonPressed'] ?? false,
            'isDoneButtonPressed': tripDoc['isDoneButtonPressed'] ?? false,
          }
        : {'isSendButtonPressed': false, 'isDoneButtonPressed': false};
  }

  Future<void> _updateButtonState(bool sendPressed, bool donePressed) async {
    await FirebaseFirestore.instance.collection('buttonStates').doc(widget.driverId).set({
      'isSendButtonPressed': sendPressed,
      'isDoneButtonPressed': donePressed,
    });
  }
            ...
            itemBuilder: (context, index) {
              ...
              return FutureBuilder<List<dynamic>>(
                ...
                builder: (context, AsyncSnapshot<List<dynamic>> snapshot) {
                  ...
                  final tripDetails = snapshot.data![1] as Map<String, dynamic>;
                          ...
                          Row(
                            ...
                            children: [
                              IconButton(
                                icon: Icon(Icons.phone),
                                onPressed: () {
                                  _launchPhoneNumber(userDetails['phone_number']);
                                },
                              ),
                              IconButton(
                                icon: Icon(Icons.map),
                              ...
                              IconButton(
                                icon: Icon(Icons.send),
                                color: isSendButtonPressed ? Colors.purple : Colors.teal,
                                onPressed: isSendButtonPressed
                                    ? null
                                    : () async {
                              ...
                              IconButton(
                                icon: Icon(Icons.done_all),
                                color: isDoneButtonPressed ? Colors.purple : Colors.teal,
                                onPressed: isSendButtonPressed
                                    ? (isDoneButtonPressed
                                        ? null
                                        : () async {
                                            await FirebaseFirestore.instance.collection('successfulTrips').add({
                                              'tripId': tripId,
                                              'driverId': widget.driverId,
                                              'userId': userId,
                                              'timestamp': FieldValue.serverTimestamp(),
                                            ...
                                            setState(() {
                                              ...
                                              _updateButtonState(true, true);
                                            });
                                            ScaffoldMessenger.of(context).showSnackBar(
                                              SnackBar(content: Text('Trip marked as successful')),
                                            );
                                          })
                                    : null,
                              ),
  ...
  Future<Map<String, dynamic>> _fetchUserDetails(String userId) async {
    ...
    return userDoc.exists ? userDoc.data()! : {'username': 'Unknown', 'phone': 'Unknown'};
  }
  ...
  Future<Map<String, dynamic>> _fetchTripDetails(String tripId) async {
    ...
    return tripDoc.exists
        ? tripDoc.data()!
        : {'pickupLocation': 'Unknown', 'deliveryLocation': 'Unknown', 'fare': '0', 'distance': '0'};
  }
  ...
  Future<void> _launchOpenStreetMapWithDirections(
      String pickupLocation, String deliveryLocation) async {
    final pickupCoords = await _geocodeAddress(pickupLocation);
    final deliveryCoords = await _geocodeAddress(deliveryLocation);

    final String openStreetMapUrl =
        'https://www.openstreetmap.org/directions?engine=graphhopper_car&route=${pickupCoords['latitude']},${pickupCoords['longitude']};${deliveryCoords['latitude']},${deliveryCoords['longitude']}';
